      ...
      <div className="flex flex-col items-center justify-center min-h-screen w-full bg-gray-100 p-6 font-sans text-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-red-100">
          ...
          <p className="text-gray-600 mb-6 text-sm leading-relaxed">
            Pour mettre cette app en ligne, connectez votre base de données{' '}
          ...
          <div className="bg-gray-50 p-4 rounded-lg text-left mb-6 border border-gray-200">
            ...
            <ol className="text-xs text-gray-600 space-y-2 list-decimal list-inside">
              ...
              <li>
                Remplacez <code>"VOTRE_API_KEY_ICI"</code> par vos clés
                Firebase.
              </li>
            </ol>
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col gap-3 mb-6">
              {selectedStartups.map((s, i) => (
                <div
                  key={i}
                  className="bg-white border border-gray-200 p-3 rounded-xl flex items-center justify-between shadow-sm"
                >
                  <span className="font-medium text-sm ml-1 truncate max-w-[120px]">
                    {s.name}
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => cycleSentiment(i)}
                      className="bg-gray-50 hover:bg-gray-100 px-3 py-1.5 rounded-lg text-lg border border-gray-200 transition-colors"
                    >
                      {s.sentiment}
                    </button>
                    <button
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full bg-gray-50 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black"
                    placeholder="Rechercher..."
                    list="startup-suggestions"
                  />
                  <datalist id="startup-suggestions">
                    {KNOWN_STARTUPS.map((s) => (
                ...
                <button
                  onClick={addStartup}
                  className="bg-gray-100 hover:bg-gray-200 text-black rounded-lg px-4 font-bold text-xl"
                >
                  +
                </button>
              </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {entry.collaborated ? (
                        (entry.startups || []).map((s, i) => (
                          <span
                            key={i}
                            className="bg-purple-50 text-purple-900 px-2 py-1 rounded text-xs border border-purple-100 flex gap-1"
                          >
                            {s.name} {s.sentiment}
                          </span>
                        ))
