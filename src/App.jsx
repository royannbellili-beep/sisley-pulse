...
import { Heart, X, Briefcase, ArrowRight, Database, BarChart2, Loader2, AlertTriangle, Settings } from 'lucide-react';

// --- 1. CONFIGURATION FIREBASE ---

// A. Configuration pour l'EXPORT (À REMPLIR quand vous hébergez ailleurs)
const exportConfig = {
...
// Nom de la collection (Adapté pour Canvas ou Prod)
const COLLECTION_NAME = isCanvasEnv 
  ? `artifacts/${appId}/public/data/sisley_pulse` // Chemin Canvas obligatoire
  : 'sisley_pulse_feedback'; // Chemin simple pour votre export
...
export default function App() {
  ...
  // Cette partie injecte automatiquement TailwindCSS s'il manque
  useEffect(() => {
  ...
  // Modifié pour accepter la clé API spécifique présente dans la config
  ...
  useEffect(() => {
    ...
    const q = collection(db, COLLECTION_NAME); // Note: En export, le chemin est simple.
    
    // Protection contre les chemins invalides
    try {
  ...
  if (!isConfigured) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6 font-sans text-center">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-red-100">
        ...
        <p className="text-gray-600 mb-6 text-sm leading-relaxed">
          L'application est prête à être exportée, mais vous devez connecter votre propre base de données <strong>Firebase</strong>.
        </p>
        ...
        <div className="bg-gray-50 p-4 rounded-lg text-left mb-6 border border-gray-200">
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-2 flex items-center gap-2">
            <Settings size={12} /> Instructions
          </h3>
          <ol className="text-xs text-gray-600 space-y-2 list-decimal list-inside">
            <li>Ouvrez le fichier <code>App.js</code> (ou <code>.jsx</code>).</li>
            <li>Cherchez l'objet <code>exportConfig</code> tout en haut.</li>
            <li>Remplacez <code>"VOTRE_API_KEY_ICI"</code> par vos vraies clés Firebase.</li>
          </ol>
        </div>
        
        <button className="w-full bg-gray-200 text-gray-500 font-bold py-3 rounded-lg cursor-not-allowed" disabled>
  ...
  if (step === 'login') return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <form onSubmit={handleLogin} className="w-full max-w-md space-y-4 bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        <div>
        ...
        <Button onClick={handleLogin} className="w-full mt-6" loading={!authUser}>
  ...
  if (step === 'swipe') return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4 relative font-sans">
      ...
      <div className={`relative w-full max-w-xs bg-white rounded-3xl shadow-2xl flex flex-col items-center justify-between p-6 border border-gray-100 transition-all duration-500 transform ${swipeDirection === 'left' ? '-translate-x-full -rotate-12 opacity-0' : ''} ${swipeDirection === 'right' ? 'translate-x-full rotate-12 opacity-0' : ''}`} style={{ minHeight: '320px' }}>
        <div className="w-full flex-1 flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-xl font-serif font-bold text-gray-800 mb-3 leading-tight">As-tu collaboré avec une Startup récemment ?</h2>
          <p className="text-gray-400 text-xs px-2">Au cours des 6 derniers mois ?</p>
        </div>
      </div>
      <div className="flex items-center gap-6 mt-6 z-10">
        <button onClick={() => handleSwipe('left')} className="w-16 h-16 rounded-full bg-white shadow-lg flex items-center justify-center text-red-500 hover:scale-110 transition-all ring-4 ring-gray-50">
          <X size={32} />
        </button>
        <button onClick={() => handleSwipe('right')} className="w-16 h-16 rounded-full bg-black shadow-lg flex items-center justify-center text-green-400 hover:scale-110 transition-all ring-4 ring-gray-200">
  ...
  if (step === 'details') return (
    <div className="min-h-screen bg-gray-50 flex justify-center font-sans">
      <div className="flex flex-col w-full max-w-md p-6 min-h-screen">
        <div className="mb-6">
          ...
          <div className="flex flex-col gap-3 mb-6">
            {selectedStartups.map((s, i) => (
              <div key={i} className="bg-white border border-gray-200 p-3 rounded-xl flex items-center justify-between shadow-sm">
                <span className="font-medium text-sm ml-1">{s.name}</span>
                <div className="flex items-center gap-2">
                  <button onClick={() => cycleSentiment(i)} className="bg-gray-50 hover:bg-gray-100 px-3 py-1.5 rounded-lg text-lg border border-gray-200">
                    {s.sentiment}
                  </button>
            ...
            {selectedStartups.length === 0 && <span className="text-gray-400 italic text-sm text-center py-4">Ajoute une startup ci-dessous...</span>}
          </div>
...
const Footer = ({ onOpenAdmin }) => (
  <footer className="fixed bottom-0 w-full bg-white border-t border-gray-200 p-3 flex justify-between items-center text-xs text-gray-400 left-0">
    <span className="pl-4">Sisley Innovation Lab v2.1</span>
    <button onClick={onOpenAdmin} className="flex items-center gap-1 hover:text-black transition-colors pr-4"><BarChart2 size={14} /> Voir les données</button>
  </footer>
);
